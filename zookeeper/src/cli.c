...
static const char *hostPort = "192.168.0.33:2181, 192.168.0.33:2182, 192.168.0.33:2183";
             ...
             void* context)
{
    ...
    if (type == ZOO_SESSION_EVENT) {
        if (state == ZOO_CONNECTED_STATE) {
			...
			fprintf(stderr, "Session expired. try to reint...\n");
			zookeeper_close(zzh);

			//Session Expired,rebuild Watcher
			//memset(&myid, 0x00, sizeof(myid));
			//zoo_set_debug_level(ZOO_LOG_LEVEL_WARN);
			//zoo_deterministic_conn_order(1); // enable deterministic order
			zh = zookeeper_init(hostPort, watcher, 30000, 0, 0, 0);
			if (!zh) {
				fprintf(stderr, "Session reint failed[%d]. Shutting down...\n", zh);
				shutdownThisThing = 1;
				zh = 0;
			}
			else {
				fprintf(stderr, "Session reint succeed[%d]. running...\n", zh);
...
#endif
    verbose = 0;
    zoo_set_debug_level(ZOO_LOG_LEVEL_WARN);//ZOO_LOG_LEVEL_WARN	ZOO_LOG_LEVEL_DEBUG
    zoo_deterministic_conn_order(1); // enable deterministic order
	//hostPort = argv[1];
